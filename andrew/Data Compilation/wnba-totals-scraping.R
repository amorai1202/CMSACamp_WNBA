...
WNBATotalStatistics <- function(season = 2018) {
...
wnba_total_stats <- tibble()
for (i in 2018:2022) {
  temp <- WNBATotalStatistics(i) %>%
    mutate(season = i)
  wnba_total_stats <- bind_rows(wnba_total_stats, temp)
}

# WRITE CSV
write_csv(wnba_total_stats, "data/wnba_total_stats.csv")
